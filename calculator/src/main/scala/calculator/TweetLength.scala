...
object TweetLength {
  ...
  def tweetRemainingCharsCount(tweetText: Signal[String]): Signal[Int] = {
    Signal[Int](MaxTweetLength - tweetLength(tweetText()))
  }

  def colorForRemainingCharsCount(remainingCharsCount: Signal[Int]): Signal[String] = {
    Signal[String](getColor(remainingCharsCount()))
  }

  private def getColor(remaingCharsCount: Int): String = {
    if (remaingCharsCount >= 15)
      "green"
    else if (remaingCharsCount >= 0 && remaingCharsCount <= 14)
      "orange"
    else
      "red"
  }
